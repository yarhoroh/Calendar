...
// [calendar app] removed unused __filename/import.meta.url (breaks in the CJS main bundle)
 ...
 */
// [calendar app] modified to accept ORT session options (e.g. executionProviders) so
// the caller can request GPU EPs with a CPU fallback (original threw on useGpu).
export async function loadTextToSpeech(onnxDir, sessionOpts = {}) {
    const cfgs = loadCfgs(onnxDir);
    const { dpOrt, textEncOrt, vectorEstOrt, vocoderOrt } = await loadOnnxAll(onnxDir, sessionOpts);
